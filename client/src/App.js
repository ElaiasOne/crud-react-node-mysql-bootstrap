  ...
  const add = () => {
    Axios.post("http://localhost:3001/create", {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: JSON.parse(JSON.stringify(error)).message==="Nerwork Error"?"Intente más tarde":JSON.parse(JSON.stringify(error)).message
      })
    });
  }
      ...
      if (result.isConfirmed) {
        Axios.delete(`http://localhost:3001/delete/${val.id}`).then(() => {
          ...
          Swal.fire({
            ...
            footer: JSON.parse(JSON.stringify(error)).message==="Nerwork Error"?"Intente más tarde":JSON.parse(JSON.stringify(error)).message
          })
        });
      }
    });
